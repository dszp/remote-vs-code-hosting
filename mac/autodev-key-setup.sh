#!/usr/bin/env bash
# Create a dedicated SSH key for a SILENT VS Code Remote-SSH host ('autodev') that does
...
# Run ON YOUR MAC.
#
#   ./mac/autodev-key-setup.sh
# Override defaults via env: OP_ACCOUNT, OP_VAULT, SSH_HOST, AUTODEV_HOST, KEY_PATH,
...
# Mac can `ssh autodev` with no challenge — that is the explicit goal here. The key is still
...
AUTODEV_HOST="${AUTODEV_HOST:-autodev}"   # the new silent Host alias to create
KEY_PATH="${KEY_PATH:-$HOME/.ssh/autodev_ed25519}"
...
ssh-keygen -t ed25519 -f "$KEY_PATH" -N "$PASS" -C "$AUTODEV_HOST-vscode-no-touchid" -q
...
if [ "$BACKUP_TO_OP" = "1" ] && [ -x "$OP" ]; then
  ...
  "$OP" item create --account "$OP_ACCOUNT" --vault "$OP_VAULT" --category "Secure Note" \
    --title "$AUTODEV_HOST SSH key (VS Code Remote-SSH)" \
    "passphrase[password]=$PASS" \
    "public key[text]=$(cat "$KEY_PATH.pub")" \
    "description[text]=Dedicated keychain-backed key for the '$AUTODEV_HOST' SSH host (silent VS Code Remote-SSH). Bypasses the 1Password agent by design (IdentityAgent none). Passphrase is also in the macOS login keychain." \
...
SHIM="$(mktemp)"; printf '#!/bin/bash\nprintf "%%s" "$AUTODEV_PASS"\n' > "$SHIM"; chmod 700 "$SHIM"
AUTODEV_PASS="$PASS" SSH_ASKPASS="$SHIM" SSH_ASKPASS_REQUIRE=force ssh-add --apple-use-keychain "$KEY_PATH"
rm -f "$SHIM"
...
>> ADD this block to ~/.ssh/config, ABOVE your 'Host *' block (ssh_config is first-match,
  ...
  Host $AUTODEV_HOST
   ...
   Then: ssh $AUTODEV_HOST   (expect a SILENT connect, even right after a resume)
   In VS Code: Remote-SSH -> '$AUTODEV_HOST'; open $WORKSPACE/<project>.
EOF
