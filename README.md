...
- `cs` folder session · `cs .` same · `cs <dir>` name+root a session after a folder, Tab-completes like `cd` (`cs Rem⇥`→`cs Remote-VS-Code`) · `cs <name>` named · `cs -n [base]` new independent (`folder-2`, …) · `cs s`/`d`/`k` pick a session to switch / detach-all-clients / kill (fzf) · `cs ls` list
