...
# Pairs with the 'autodev' silent SSH host (mac/autodev-key-setup.sh).

# Open a VM workspace folder in a NEW VS Code window via Remote-SSH (silent 'autodev').
...
rcode() {
  local host="${RCODE_HOST:-autodev}" arg="${1:-}" folder
...
rpaste() {
  local host="${RCODE_HOST:-autodev}" dir="${RPASTE_DIR:-/home/__DEV_USER__/.cache/pastes}"
